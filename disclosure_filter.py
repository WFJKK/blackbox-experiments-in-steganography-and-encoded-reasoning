...
def detect_disclosure(text: str, payload: str) -> tuple[bool, str | None]:
...
def main() -> None:
    ...
    parser.add_argument(
        "--verbose", action="store_true", help="Print disclosed trials with reason"
    )
    ...
    groups = defaultdict(
        lambda: {"n": 0, "disclosed": 0, "clean_ber_sum": 0, "clean_n": 0, "ber_sum": 0}
    )

    for r in results:
        if "error" in r:
            continue
        disclosed, reason = detect_disclosure(
            r.get("encoded_response", ""), r["payload"]
        )
        ...
        if disclosed:
            ...
            if args.verbose:
                print(
                    f"DISCLOSED: {r['model']}/{r['scheme']}/trial{r['trial']} payload={r['payload']}  ({reason})"
                )
        else:
            g["clean_n"] += 1
    ...
    print()
    print(
        f"{'Model':<20} {'Scheme':<12} {'N':>4} {'Disclosed':>10} {'Pay(all)':>9} {'Pay(clean)':>11} {'N_clean':>8}"
    )
    print("-" * 80)
    for (m, s), g in sorted(groups.items()):
        ...
        print(
            f"{m:<20} {s:<12} {g['n']:>4} {g['disclosed']:>3}/{g['n']:<6} "
