...
def load_math500(cache_dir: str = "data") -> list[dict[str, Any]]:
    ...
    if os.path.exists(cache_file):
        ...
        if len(problems) < 500:
            print(
                f"WARNING: only {len(problems)} problems in {cache_file} (expected 500)."
            )
...
def sample_problems(
    problems: list[dict[str, Any]], n: int, seed: int = 42
) -> list[dict[str, Any]]:
