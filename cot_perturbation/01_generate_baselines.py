...
from utils.vllm_client import (
    get_vllm_client, get_served_model,
    generate_baseline, parse_cot_and_answer,
)
...
def main():
    ...
    parser.add_argument("--n", type=int, default=N_BASELINES, help="Number of baselines")
    ...
    try:
        ...
        print(f"Start it with: vllm serve {model_config['hf_id']} --port {args.vllm_port}")
        sys.exit(1)

    if model_config["hf_id"] not in served:
        print(f"WARNING: vLLM is serving '{served}', expected '{model_config['hf_id']}'")
    ...
    for problem in problems:
        ...
        print(f"  CoT: {len(cot.split())} words | answer: {answer[:80]!r} | {elapsed:.1f}s")
